...
// =============================================================================
/**
 * Bottom sheet for managing user account, appearance, app updates, legal links, and build information.
 *
 * Uses GlassBottomSheet for Liquid Glass styling with imperative API.
 * Provides UI and handlers for editing display name, signing out, deleting account, selecting theme mode,
...
const SettingsSheet = forwardRef<SettingsSheetRef>((props, ref) => {
   ...
   *
   * Note: We dismiss the sheet first, then sign out. The root layout's auth guard
  ...
  const handleSignOut = async () => {
    if (Platform.OS === 'web') {
      ...
      if (confirmed) {
        try {
          // Dismiss sheet first, then sign out
