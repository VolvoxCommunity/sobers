...
describe('Documentation and discoverability', () => {
  it('should have events grouped logically by category', () => {
    ...
    // Onboarding events: 7
    const onboardingEvents = Object.keys(AnalyticsEvents).filter((k) =>
    ...
    // Social events: 6
    const socialEvents = Object.keys(AnalyticsEvents).filter(
      (k) => k.startsWith('SPONSOR_') || k.startsWith('SPONSEE_') || k.startsWith('MESSAGE_')
    );
    expect(socialEvents).toHaveLength(6);

    // App/Engagement events: 4
    const appEvents = Object.keys(AnalyticsEvents).filter(
      (k) => k.startsWith('APP_') || k === 'DAILY_CHECK_IN'
    );
